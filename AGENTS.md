...
Package manager: **Yarn 4** (`packageManager` is pinned; `yarn.lock` is
canonical). A stray `package-lock.json` is present and should be removed - do not
use npm.
...
- Typecheck: `yarn check` (`tsc --noEmit`)
- Test: `yarn test` (`vitest run`)
- Format: `yarn format` / `yarn format:check`

The Husky pre-commit hook runs `yarn lint` + `yarn tsc --noEmit`.

**Testing gate is ON** (`yarn test` is declared above): a step that adds pure
logic in `src/utils/` ships a passing test in the same diff, and `yarn test`
must be green before a step is approved and before `/complete` merges. Scope and
exclusions are in `blueprint/context/coding-standards.md`.

No combined `Verify` command and no CI yet. Run `/ci` when you want one Verify
command plus matching automatic GitHub checks.

Browser testing is opt-in and not set up. Run `/browser-tests` if a repeatable
harness becomes worthwhile; do not add one mid-feature.

### Database

- Migrations: `supabase/migrations/<utc-timestamp>_<name>.sql`, applied with the
  Supabase CLI (`yarn supabase migration up` locally, `supabase db push` to the
  linked remote).
- Local reset (re-run all migrations + `supabase/seed.sql`):
  `npx supabase db reset --local`.
- Never seed `auth.users` / passwords / environment rows in a migration.
