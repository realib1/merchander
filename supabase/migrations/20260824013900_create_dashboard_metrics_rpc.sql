-- Create a robust RPC to calculate dashboard metrics in the database
...
begin
  ...
  select 
    ...
    count(distinct o.id),
    coalesce(sum(oi.quantity * (oi.unit_price * 0.6)), 0) -- fallback cost to 60% of unit price
  ...
  select 
    coalesce(sum(o.total_amount - o.delivery_fee), 0),
    count(distinct o.id),
    coalesce(sum(oi.quantity * (oi.unit_price * 0.6)), 0)
