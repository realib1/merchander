...
import { ArrowLeftRight, Check, Trash2, Search, Plus, ChevronDown } from 'lucide-react';
...
function StockTransferContent({
  ...
  const [sourceStoreId, setSourceStoreId] = useState<string>(
    initialSourceBranch?.id || branches[0]?.id || ''
  );
  ...
  return (
    <form id="stock-transfer-drawer-form" onSubmit={handleSubmit} className="space-y-4">
      {/* Branch Selectors */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            className="w-full bg-surface border border-separator rounded-xl px-3 py-2 text-xs outline-none focus-visible:ring-1 focus-visible:ring-brand-primary font-medium"
          >
            {branches.map((b) => (
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        <div
          ...
          className="flex items-center gap-2 bg-surface border border-separator rounded-xl px-3 py-2 text-xs cursor-pointer focus-within:ring-1 focus-within:ring-brand-primary"
        >
          ...
          <input
            ...
            className="w-full bg-transparent outline-none placeholder:text-muted/60 text-xs text-foreground"
          />
        ...
        {/* Dropdown Popover */}
        {isDropdownOpen && (
          <div className="absolute top-full left-0 right-0 mt-1.5 bg-surface-elevated border border-separator rounded-xl shadow-xl z-50 max-h-52 overflow-y-auto divide-y divide-separator/50 custom-scrollbar animate-fadeIn">
      ...
      <div>
        ...
        {batchItems.length === 0 ? (
          <div className="p-6 text-center border border-dashed border-separator rounded-xl bg-surface/40 space-y-1">
      ...
      <div className="flex items-center justify-between gap-3 pt-4 border-t border-separator/50">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isPending ? 'Transferring...' : `Transfer ${totalUnits} Units`}
          </Button>
        </div>
      </div>
    </form>
...
export function StockTransferDrawer({
  ...
  return (
    <Drawer
      ...
      description="Reallocate multiple items between physical branches."
      size="lg"
    >
      {isOpen && (
        <StockTransferContent
          key={initialSourceBranch?.id || 'default-transfer'}
          branches={branches}
