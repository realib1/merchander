...
export default async function BusinessProfileSettingsPage() {
  ...
  // Fetch tenant info
  const { data: tenantUsers } = await supabase.from('tenant_users').select('tenant_id').eq('user_id', user.id).single();

  let tenantName = '';
  if (tenantUsers?.tenant_id) {
    const { data: tenant } = await supabase.from('tenants').select('name').eq('id', tenantUsers.tenant_id).single();
    if (tenant) {
  ...
  return (
    <div className="max-w-3xl space-y-8 animate-fadeIn">
      <div>
        ...
        <p className="text-sm  mt-1">Manage your company&apos;s legal information and contact details.</p>
      </div>
