        ...
        <tbody className="divide-y divide-separator">
          {purchaseOrders.map((po) => {
            ...
            <tr key={po.id} className="hover:bg-surface-elevated/30 transition-colors">
              ...
              <td className="px-4 py-3">
                <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-caption font-semibold ${
                  po.status === 'received' ? 'bg-emerald-500/10 text-emerald-600' : 
                  po.status === 'ordered' ? 'bg-blue-500/10 text-blue-600' : 'bg-orange-500/10 text-orange-600'
                }`}>
