        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-separator">
              ...
              <button 
                onClick={onClose}
                className="p-2 hover:bg-surface-elevated rounded-lg transition-colors text-secondary"
              >
                <X size={20} />
              </button>
            </div>
              ...
              <div>
                ...
                <div className="bg-surface-elevated p-4 rounded-xl border border-separator space-y-3">
                  <div>
                    ...
                    <div className="text-sm font-semibold text-primary">{order.customer?.name || 'Unknown'}</div>
                  </div>
                  <div>
                  ...
                  {order.delivery_address && (
                    <div>
                      <div className="text-xs text-muted flex items-center gap-1"><MapPin size={12} /> Address</div>
                      <div className="text-sm text-primary mt-0.5">{order.delivery_address}</div>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <ul className="divide-y divide-separator">
                    {order.items?.map((item) => (
                      <li key={item.id} className="p-4 flex items-center justify-between">
                        <div>
                          <div className="text-sm font-semibold text-primary">{item.variant?.name || 'Product'}</div>
              ...
              <div>
                ...
                <div className="bg-surface-elevated p-4 rounded-xl border border-separator space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-primary font-medium">{formatCurrency((order.total || 0))}</span>
                  </div>
                  <div className="pt-2 border-t border-separator mt-2 flex justify-between">
                    <span className="font-semibold text-primary">Total</span>
                    <span className="font-bold text-primary font-display">{formatCurrency(order.total || 0)}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 border-t border-separator bg-surface-elevated/50 flex gap-3">
              <button 
                onClick={onClose}
                className="flex-1 px-4 py-2 bg-surface text-primary border border-separator rounded-xl text-sm font-semibold hover:bg-surface-elevated transition-colors"
              >
                Close
              </button>
            </div>
