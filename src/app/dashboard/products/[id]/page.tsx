...
export default async function ProductDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Calculate totals
  const totalStock =
    product.variants?.reduce((acc: number, v: VariantWithStore) => {
      const inv =
        v.inventory?.reduce(
          (iAcc: number, i: { quantity: number; store?: { name: string } }) => iAcc + (i.quantity || 0),
          0
        ) || 0;
      return acc + inv;
    }, 0) ?? 0;
    ...
    <div className="min-h-full flex flex-col max-w-5xl mx-auto w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {product.is_active ? (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-caption font-semibold bg-emerald-500/10 text-emerald-600">
                  Active
                </span>
              ) : (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-caption font-semibold bg-orange-500/10 text-orange-600">
                  Archived
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="px-4 py-2 bg-surface border border-separator  rounded-xl font-medium text-sm hover:bg-surface-elevated transition-colors flex items-center gap-2"
          >
            <Edit size={16} />
            Edit Product
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              {product.image_urls && product.image_urls.length > 0 && (
                <div className="col-span-2 sm:col-span-1">
                  <div className="relative aspect-square rounded-xl overflow-hidden border border-separator bg-surface-elevated/30">
              ...
              <div
                className={`col-span-2 ${product.image_urls && product.image_urls.length > 0 ? 'sm:col-span-1' : ''} space-y-4`}
              >
                <div>
          ...
          <div className="bg-surface border border-separator rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-separator">
              {product.variants?.map((variant: VariantWithStore) => {
                const variantStock =
                  variant.inventory?.reduce((acc: number, i: { quantity: number }) => acc + (i.quantity || 0), 0) || 0;

                return (
                  <div key={variant.id} className="p-4 sm:p-6 hover:bg-surface-elevated/20 transition-colors">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
                      <div>
                        <div className="font-semibold  mb-1">{variant.name || product.name}</div>
                        <div className="text-xs font-mono text-muted bg-surface-elevated px-2 py-1 rounded inline-block">
                          {variant.sku}
                        </div>
                      </div>
                      <div className="flex items-center gap-6">
                        ...
                        <div className="text-right">
                          ...
                          <div
                            className={`font-bold ${variantStock === 0 ? 'text-red-500' : variantStock < 10 ? 'text-orange-500' : 'text-emerald-600'}`}
                          >
                    ...
                    {variant.inventory && variant.inventory.length > 0 && (
                      <div className="mt-4 bg-surface-elevated/30 rounded-xl p-3 border border-separator/50">
                        <div className="text-xs font-medium  mb-2 uppercase tracking-wider">Inventory Breakdown</div>
        ...
        <div className="space-y-6">
          <div className="bg-surface border border-separator rounded-2xl p-6 shadow-sm">
            <h3 className="text-sm font-semibold mb-4 uppercase tracking-wider text-muted">Summary</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-medium  text-sm">{new Date(product.created_at).toLocaleDateString()}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-medium  text-sm">{product.variants?.length || 0} variant(s)</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {product.vendor && (
            <div className="bg-surface border border-separator rounded-2xl p-6 shadow-sm">
              <h3 className="text-sm font-semibold mb-4 uppercase tracking-wider text-muted">Vendor</h3>
