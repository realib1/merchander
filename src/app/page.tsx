...
import { Ship, Wallet, MessageSquare, LineChart, Package, CheckCircle, Store, Box, Briefcase, Truck, ArrowBigRight } from "lucide-react";
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
          ...
          <div className="w-full max-w-5xl">
            <FadeInView delay={0.4}>
              <div className="text-center mb-10">
                <h2 className="text-sm font-bold tracking-widest text-brand-primary uppercase">Your business, at a glance.</h2>
              </div>
                      ...
                      <div className="flex-1 flex items-end justify-between gap-2 pb-1">
                        {[40, 70, 45, 90, 65, 80, 100].map((h, i) => (
                          <div key={i} className="w-full bg-brand-primary/20 rounded-t-sm transition-all hover:bg-brand-primary/40" style={{ height: `${h}%` }}>
                            {i === 6 && <div className="w-full h-full bg-brand-primary rounded-t-sm shadow-[0_0_15px_rgba(245,158,11,0.5)]"></div>}
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <section id="demo" className="relative px-4 py-24 sm:px-6 lg:px-8 border-b border-separator overflow-hidden bg-background">
          {/* Dotted Grid Background pattern */}
          <div className="absolute inset-0 z-0 opacity-20 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at center, var(--color-separator) 1px, transparent 1px)', backgroundSize: '24px 24px' }}></div>
