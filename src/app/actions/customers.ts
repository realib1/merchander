...
export interface CustomerStats {
  ...
  first_touch_source?: CustomerAttributionSource;
...
export async function getCustomerAttributionBreakdownAction(): Promise<CustomerAttributionBreakdown> {
  ...
  try {
    ...
    if (!tenantUser) return emptyResult;

    const [customersRes, ordersRes] = await Promise.all([
      supabase.from('customers').select('id, first_touch_source').eq('tenant_id', tenantUser.tenant_id),
      supabase
        .from('orders')
        .select('customer_id, total_amount, status, attribution_source')
        .eq('tenant_id', tenantUser.tenant_id)
